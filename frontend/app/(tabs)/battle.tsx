...
export default function Battle() {
  ...
  return (
    <ImageBackground source={{ uri: IMAGES.bgMystical }} style={styles.bg}>
      ...
      <View style={styles.screen} testID="battle-screen">
        ...
        <View style={styles.heroCard}>
          <View style={{ flex: 1 }}>
            ...
            <View style={styles.classLine}><Text style={styles.primaryClass}>Primary: {heroChip?.icon ?? "⚔️"} {heroChip?.label ?? "Infantry"}</Text>{secondaryChip && <Text style={styles.secondaryClass}>Sub: {secondaryChip.icon} {secondaryChip.label}</Text>}<TouchableOpacity style={styles.affinityButton} onPress={() => setAffinityOpen(true)}><Text style={styles.affinityButtonText}>ⓘ Affinity</Text></TouchableOpacity></View>
          </View>
      ...
      <Modal visible={affinityOpen} transparent animationType="fade" onRequestClose={() => setAffinityOpen(false)}><View style={styles.modalBackdrop}><View style={styles.infoCard}><Text style={styles.infoTitle}>Class Affinity</Text><Text style={styles.infoBody}>Your class is dynamic. Equipment and battle choices raise or lower these affinities over time.</Text>{Object.entries(classState?.scores ?? {}).sort((a,b) => b[1]-a[1]).map(([key, score]) => <View key={key} style={styles.affinityRow}><Text style={styles.infoRuleLabel}>{key.replace(/_/g, " ").toUpperCase()}</Text><Text style={styles.infoRuleValue}>{score}</Text></View>)}<Text style={styles.infoTip}>Highest score becomes your Primary class. Second highest becomes your Sub affinity.</Text><TouchableOpacity style={styles.closeInfo} onPress={() => setAffinityOpen(false)}><Text style={styles.closeInfoText}>CLOSE</Text></TouchableOpacity></View></View></Modal>
...
const styles = StyleSheet.create({
  ...
  affinityButton: { borderWidth: 1, borderColor: COLORS.borderStrong, borderRadius: 999, paddingHorizontal: 7, paddingVertical: 2, backgroundColor: "rgba(0,229,255,0.08)" }, affinityButtonText: { color: COLORS.secondary, fontSize: 9, fontWeight: "900" }, affinityRow: { flexDirection: "row", justifyContent: "space-between", borderTopWidth: 1, borderTopColor: COLORS.border, paddingTop: 9, marginTop: 9 }, outcomeButtons: { flexDirection: "row", gap: 8, marginTop: 10 }, outcomePrimaryBtn: { backgroundColor: COLORS.primary, borderRadius: 12, paddingHorizontal: 18, paddingVertical: 10 }, outcomeBtnText: { color: "white", fontWeight: "900", letterSpacing: 1 },
