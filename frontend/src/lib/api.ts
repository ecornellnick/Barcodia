...
export type Item = {
  ...
  rarity: "common" | "uncommon" | "rare" | "epic" | "legendary"; level: number; element: Element;
...
export type User = {
  ...
  difficulty_tier: number; battles_won: number; battles_lost: number; class_affinity?: Record<string, number>;
};
...
export const RARITY_GLOW: Record<Item["rarity"], string> = {
  common: "rgba(113,128,150,0.35)",
  uncommon: "rgba(56,161,105,0.48)",
  rare: "rgba(49,130,206,0.55)",
  epic: "rgba(128,90,213,0.65)",
  legendary: "rgba(221,107,32,0.75)",
};
...
export function itemIcon(item?: Partial<Item> | null): string {
  ...
  if (item.slot === "main_hand") {
    if (tags.includes("archer") || shape === "ranged" || name.includes("bow")) return "🏹";
    if (tags.includes("lancer") || shape === "long" || /(lance|spear|halberd|pike)/.test(name)) return "🔱";
    if (tags.includes("mage") || tags.includes("healer") || /(staff|wand|tome|rod|orb)/.test(name)) return "🪄";
    if (tags.includes("assassin") || /(dagger|knife|stiletto|dirk)/.test(name)) return "🗡️";
    if (/(mace|hammer|maul|cudgel)/.test(name)) return "🔨";
    if (/(axe|cleaver)/.test(name)) return "🪓";
    return "⚔️";
  }
  if (item.slot === "off_hand") {
    if (/(shield|buckler|ward)/.test(name)) return "🛡️";
...
export function itemPrimaryPower(item?: Partial<Item> | null): number {
  if (!item) return 0;
  return Math.max(Number(item.atk || 0), Number(item.int_stat || 0));
}
export function itemDefensePower(item?: Partial<Item> | null): number {
  if (!item) return 0;
  return Number(item.def_stat || 0) + Number(item.res || 0);
}
export function itemStatTotal(item?: Partial<Item> | null): number {
  if (!item) return 0;
  const keys = ["atk", "int_stat", "def_stat", "res", "dex", "mob", "crit", "luk", "hp", "mana", "stamina_restore", "upgrade_xp_value"] as const;
  return keys.reduce((sum, k) => sum + Number((item as any)[k] || 0), 0);
}
